<?php
class New_Model_Product extends Louis_Db_Table_Abstract 
     {
       protected $_name    = 'product';
       protected $_primary  = 'id';
       protected $_content_type  = 'new';
       ...
       public function init(){
        ...
        $search = $this->get_array_value($options, "search");
        
       if($search){
	         $select->where('p.name LIKE ?', "%{$search}%");	        
	         $select->orWhere('p.description LIKE ?', "%{$search}%");
