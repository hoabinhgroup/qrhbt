...
class Louis_Slim {
    ...
    public static function getImages($inputName = 'slim') {
        ...
        $values = Louis_Slim::getPostData($inputName);
        ...
        foreach ($values as $value) {
            $inputValue = Louis_Slim::parseInput($value);
            if ($inputValue) {
    ...
    private static function parseInput($value) {
        ...
        if (isset ($data->input)) {
            ...
            if (isset($data->input->image)) {
                $inputData = Louis_Slim::getBase64Data($data->input->image);
            }
        ...
        if (isset($data->output)) {
            ...
            if (isset($data->output->image)) {
                $outputData = Louis_Slim::getBase64Data($data->output->image);
            }
    ...
    public static function saveFile($data, $name, $path = 'tmp/', $uid = true) {
        ...
        $name = Louis_Slim::sanitizeFileName($name);
